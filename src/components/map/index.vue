<template>
  <div class="mt-4 rounded-2xl bg-base-100">
    <div class="d-flex h-20 ai-center jc-between">
      <div class="p-4 text-2xl italic">
        个人足迹
      </div>
...
mapboxgl.accessToken = 'pk.eyJ1IjoienhsYmVuIiwiYSI6ImNsNWh2ZnBrYTAwbmEzZmttc3VrcjFxdmwifQ.ow_IFMv7HHXeLSSNv9YNjw';
...
// Insert the layer beneath any symbol layer.
    const layers: Array<any> = map.value.getStyle().layers;
...
const addMakerImage = (map: Ref<Map>) => {
  ...
  for (const markeri of geojson.features) {
    ...
    new mapboxgl.Marker(el)
        .setLngLat(markeri.geometry.coordinates)
        .setPopup(popup)
        .addTo(map.value);
  }
}
...
const increase = (map: Ref<Map>) => {
  map.value.on('load', () => {
    ...
    map.value.on('click', 'places', function (e: any) {
      console.log(e, 111);
